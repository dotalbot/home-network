...
| Node exporter hardening | TCP `9100` is live and staged hardening exists; stage 07 was run on `jellybase`, `jellyhome`, and `jellyberry` but UFW was inactive on all three, so no rules were applied and negative verification still fails open | explicit operator decision to enable UFW or apply equivalent nftables/iptables allowlist, then generated `stage-07-configure-access-control.sh` plus positive/negative verification |
...
1. Decide whether to enable UFW or apply equivalent nftables/iptables node_exporter TCP `9100` allowlist rules on `jellybase`, `jellyhome`, and `jellyberry`; stage 07 has already been run and intentionally made no changes while UFW was inactive.
