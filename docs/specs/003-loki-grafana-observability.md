...
- Loki becomes the log-history layer.
...
- Netdata is not the strategic monitoring path and should not receive new streaming work unless that decision is explicitly reversed.

## Initial scope

1. Deploy self-hosted Loki on `jellybase` beside Prometheus and Grafana.
2. Provision Loki as a Grafana datasource through repo-managed config.
3. Send Borgmatic run logs to Loki using Borgmatic's Loki monitoring hook.
...
- Loki is reachable from the trusted monitoring path on `jellybase`.
- Grafana lists a provisioned Loki datasource.
- At least one Borgmatic run produces queryable Loki entries with expected host/job labels.
- Documentation explains rollback and how to verify logs from CLI and Grafana.
- No secrets, repository URLs with credentials, or raw passphrases appear in labels or committed config.
